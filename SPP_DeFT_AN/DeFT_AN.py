...
class Network(nn.Module):
    def __init__(self, mic_num=6, ch_dim=64, win=256, num_head=2, dropout=0.1, num_layer=2, depth=3):
        ...
        self.win = win
        self.hop = self.win // 2
        self.window = torch.hamming_window(self.win)
        ...
        self.out_conv = nn.Conv2d(in_channels=self.ch_dim, out_channels=1, kernel_size=(5, 5), padding=(2, 2))
    ...
    def forward(self, input):
        B, M, F, L, _ = input.size()
        xi = input.permute(0, 1, 4, 3, 2).contiguous()             # B, M, 2, L, F
        xi = xi.view([B, M*2, L, F])                            # B, 2*M, L, F

        xo = self.inp_conv(xi)                                  # B, C, L, F
        for idx in range(self.num_layer):
            xo = self.proposed_block[idx](xo)                   # B, C, L, F
        mask = torch.sigmoid(self.out_conv(xo))
        mask = mask.transpose(2, 3).squeeze(1)
